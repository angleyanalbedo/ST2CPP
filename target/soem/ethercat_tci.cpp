/**
 * ethercat_tci.cpp — EtherCAT TCI 实现（SOEM context-based API）
 */
...
void EthercatTCI::syncInputs(ProcessImage& img) {
    ...
    for (int i = 0; i < ECAT_INPUT_MAP_SIZE; i++) {
        ...
        // SOEM: slave 的 input 数据在 slavelist[slaveIdx].inputs 指针
        ...
        if (m.plcBitOff >= 0) {
            ...
            if (m.plcByteOff + m.sizeBytes <= PROCESS_IMAGE_SIZE) {
